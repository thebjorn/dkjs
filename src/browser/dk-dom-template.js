...
dom.Template = dk.Class.extend({
    ...
    ctor: function (props, domtemplates, name, position) {
        var self = this;
        ...
        var tmptemplate = props.template;
        ...
        var subitems = _.difference(Object.keys(props), Object.keys(this));
        subitems.forEach(function (item, position) {
            self.structure[item] = dom.Template.Kcreate(props[item], domtemplates, item.replace('-', '_'), position);
        });
     ...
     *
     */
    construct_on: function (location, creator, accessor) {
        var domitem = dom.DomItem.create(this);
        return domitem.construct_on(location, creator, accessor);
    },

    append_to: function (location, creator, accessor) {
        var self = this;
        if (accessor === undefined) accessor = {};
        var items = [];
        Object.keys(this.structure).forEach(function (key) {
            var domitem = dom.DomItem.create(self.structure[key]);
...
dom.DomItem = dk.Class.extend({
     ...
     */
    ctor: function (template, parentitem) {
        var self = this;
        ...
        if (this.template.structure) {
            ...
            var vals = this.keys.map(function (key) {
                return dom.DomItem(template.structure[key], self);
            });
    ...
    _insert_item: function (item, pos) {
        if (pos === 0) {
    ...
    _make_item: function (creator) {
        var maker = (creator && creator.layout)? creator.layout: {make: function (t) { return $(t); }};
        var item = maker.make(this.template.template);
        return this._construct_item(item);
    },

    _construct_item: function (item) {
    ...
    append: function (location, creator) {
        this.location = (location && location.jquery)? location: $(location);
        var item = this._make_item(creator);
